...
// у SITE_ADAPTERS нижче і, за потреби, новий запис у manifest.json → content_scripts.matches.
...
(() => {
  ...
  const { buildCaptionCandidates, parseTimedtextBody, pollForMatchingTracks } = self.YouTubeCaptions;
  ...
  function requestCaptionTracksFromBridge() {
  ...
  async function fetchYouTubeCues() {
  ...
  function startDomCaptionFallback(onLineChange) {
  ...
  const YouTubeAdapter = {
    ...
    getMountPoint(video) {
      return document.getElementById('movie_player') || (video && video.parentElement);
    },

    getFullscreenRoot(video) {
    ...
    // вставити, — settingsPanel.js звіряється з цим перед реінʼєкцією.
