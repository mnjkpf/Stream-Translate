...
function cleanCueText(raw: string): string {
  return raw
    .replace(/<[^>]+>/g, '')
    .replace(/\{[^}]+\}/g, '')
    // Маркери зміни мовця зі стандарту субтитрів (CEA-608) і ASR YouTube:
    // '>>' — заговорив інший, '>>>' — нова тема. Це службова розмітка, а не
    // текст репліки: у перекладі вона лише шумить, а в режимі слова її можна
    // випадково клікнути. Прибираємо разом із подвоєними пробілами по слідах.
    .replace(/>{2,}/g, ' ')
    .replace(/[ \t]{2,}/g, ' ')
    .trim();
}
